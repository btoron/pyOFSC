...
def _create_response(method: str, url: str, **kwargs) -> MockResponse:
    ...
    if method.upper() == "DELETE":
        ...
        return MockResponse(status_code=204, json_data={}, text_data="", elapsed_ms=50)
    elif method.upper() == "POST":
        # POST requests typically return 201 Created
        return MockResponse(
            status_code=201, json_data=_mock_config["default_response"], elapsed_ms=150
        )
    else:
...
def put(
    ...
    **kwargs,
    ...
    Returns:
        MockResponse object
    """
    return _create_response("PUT", url, data=data, json=json, headers=headers, **kwargs)
...
def delete(
    url: str, headers: Optional[Dict[str, str]] = None, **kwargs
) -> MockResponse:
...
def setup_ofsc_mock_responses():
    ...
    # Core endpoints
    set_mock_response("/rest/ofscCore/", status_code=200, json_data={"success": True})

    # Capacity endpoints
    set_mock_response(
        "/rest/ofscCapacity/", status_code=200, json_data={"success": True}
    )
