...
class OFSCResponseCollector:
    ...
    async def collect_response(
        self, endpoint_id: int, path: str, description: str
    ) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                try:
                    ...
                    response_data["error"] = response.text
                print(
                    "❌ Not Found (404): Endpoint may not exist or requires valid identifier"
                )
                self._save_response(endpoint_id, description, response_data)
    ...
    def _get_existing_labels(
        self, response_file: str, label_field: str = "label"
    ) -> List[str]:
        ...
        try:
            with open(filepath, "r") as f:
                ...
                return [
                    item.get(label_field) for item in items if item.get(label_field)
                ]
        except:
            return []
...
async def main():
    ...
    async with OFSCResponseCollector() as collector:
        ...
        # Endpoint #27: Forms
        success = await collector.collect_response(
            27, "/rest/ofscMetadata/v1/forms", "get_forms"
        )
        if not success:
        ...
        success = await collector.collect_response(
            35, "/rest/ofscMetadata/v1/linkTemplates", "get_link_templates"
        )
        if not success:
        ...
        success = await collector.collect_response(
            57, "/rest/ofscMetadata/v1/routingProfiles", "get_routing_profiles"
        )
        if not success:
        ...
        if shift_labels:
            ...
            success = await collector.collect_response(
                64, f"/rest/ofscMetadata/v1/shifts/{shift_label}", "get_shift"
            )
            if not success:
        ...
        workzone_labels = collector._get_existing_labels(
            "78_workzones.json", "workZoneLabel"
        )
        if workzone_labels:
            workzone_label = workzone_labels[0]  # Use first available workzone
            success = await collector.collect_response(
                82, f"/rest/ofscMetadata/v1/workZones/{workzone_label}", "get_workzone"
            )
            if not success:
        ...
        link_template_labels = collector._get_existing_labels(
            "35_get_link_templates.json", "label"
        )
        if link_template_labels:
            template_label = link_template_labels[0]
            success = await collector.collect_response(
                36,
                f"/rest/ofscMetadata/v1/linkTemplates/{template_label}",
                "get_link_template",
            )
            if not success:
        ...
        if map_layer_data and map_layer_data.get("totalResults", 0) > 0:
            map_layer_labels = [
                item.get("label")
                for item in map_layer_data.get("items", [])
                if item.get("label")
            ]
            if map_layer_labels:
                ...
                success = await collector.collect_response(
                    41,
                    f"/rest/ofscMetadata/v1/mapLayers/{layer_label}",
                    "get_map_layer",
                )
                if not success:
            ...
            print(
                "⚠️  No map layers available for individual endpoint (empty collection)"
            )

        # Endpoint #58 & #59: Routing Profile Plans (if we got routing profiles)
        routing_profile_labels = collector._get_existing_labels(
            "57_get_routing_profiles.json", "label"
        )
        if routing_profile_labels:
            profile_label = routing_profile_labels[0]
            success = await collector.collect_response(
                58,
                f"/rest/ofscMetadata/v1/routingProfiles/{profile_label}/plans",
                "get_routing_profile_plans",
            )
            if success:
                # Try to get plan export if plans exist
                plan_labels = collector._get_existing_labels(
                    "58_get_routing_profile_plans.json", "label"
                )
                if plan_labels:
