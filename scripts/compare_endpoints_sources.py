...
def compare_implementation_status(registry_endpoint: EndpointInfo, doc_endpoint: EndpointDoc) -> List[str]:
    ...
    if registry_implemented and not doc_has_version:
        issues.append(f"✅ Registry shows implemented ({registry_endpoint.implemented_in}) but doc shows not implemented")
    elif not registry_implemented and doc_has_version:
        issues.append(f"❌ Doc shows implemented (version: {doc_endpoint.implemented_in}) but registry shows not implemented")
...
def compare_signatures(registry_endpoint: EndpointInfo, doc_endpoint: EndpointDoc) -> List[str]:
    ...
    if registry_sig and doc_sig:
        if registry_sig != doc_sig:
            issues.append(f"📝 Signature mismatch:")
...
def compare_basic_info(registry_endpoint: EndpointInfo, doc_endpoint: EndpointDoc) -> List[str]:
    """Compare basic endpoint information."""
    issues = []
    
    if registry_endpoint.path != doc_endpoint.path:
        issues.append(f"🛤️ Path mismatch: Registry='{registry_endpoint.path}' vs Doc='{doc_endpoint.path}'")
    
    if registry_endpoint.method != doc_endpoint.method:
        issues.append(f"📋 Method mismatch: Registry='{registry_endpoint.method}' vs Doc='{doc_endpoint.method}'")
    
    if registry_endpoint.module != doc_endpoint.module:
        issues.append(f"📦 Module mismatch: Registry='{registry_endpoint.module}' vs Doc='{doc_endpoint.module}'")
...
def generate_comparison_report():
    ...
    print("=" * 80)
    print(f"Comparing ENDPOINTS.md with endpoints_registry.py")
    print()
    ...
    for endpoint_id in sorted(set(doc_endpoints.keys()) | set(registry_by_id.keys())):
        ...
        if not doc_endpoint:
            endpoint_issues.append(f"📋 Missing in ENDPOINTS.md")
            missing_in_doc += 1
        elif not registry_endpoint:
            endpoint_issues.append(f"🗂️ Missing in endpoints_registry.py")
    ...
    if total_issues == 0:
        ...
        if missing_in_registry > 0:
            print(f"  2. Investigate {missing_in_registry} endpoints missing from registry")
        if implementation_issues > 0:
            print(f"  3. Sync implementation status for {implementation_issues} endpoints")
