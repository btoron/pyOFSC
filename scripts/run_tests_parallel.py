...
def determine_worker_count(test_category: str, total_cpus: int) -> int:
    ...
    disable_parallel = os.environ.get('PYTEST_DISABLE_PARALLEL', '').lower() in ('true', '1', 'yes')
    if disable_parallel:
        return 1
...
Examples:
  ...
  %(prog)s --custom tests/unit/    # Run custom test path
    ...
    test_group.add_argument('--models', action='store_true',
                           help='Run model tests only')
    test_group.add_argument('--integration', action='store_true',
                           help='Run integration tests only')
    test_group.add_argument('--end-to-end', action='store_true',
                           help='Run end-to-end tests only')
    test_group.add_argument('--live', action='store_true',
                           help='Run live tests only')
    test_group.add_argument('--custom', nargs='+',
                           help='Run tests from custom paths')
    ...
    parser.add_argument('--measure', action='store_true',
                       help='Measure and report execution time')
    ...
    if args.unit:
        ...
        test_categories.append(('live', ['tests/live/', 'tests/end_to_end/', '-m', 'live']))
    elif args.custom:
