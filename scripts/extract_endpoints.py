...
def _extract_method_parameters(method_node: ast.AsyncFunctionDef) -> List[Dict[str, Any]]:
    ...
    if args.vararg:
        parameters.append({
            ...
            'annotation': _ast_to_string(args.vararg.annotation) if args.vararg.annotation else None,
            'default': None,
            'is_optional': False
        })
    ...
    if args.kwarg:
        parameters.append({
            ...
            'annotation': _ast_to_string(args.kwarg.annotation) if args.kwarg.annotation else None,
            'default': None,
            'is_optional': False
        })
    ...
    try:
        ...
        if isinstance(node, ast.Name):
            ...
            if (isinstance(node.value, ast.Attribute) and 
                ...
                node.value.value.id == 'ExportMediaType' and
                node.value.attr == 'OCTET_STREAM' and
                node.attr == 'value'):
                return '"application/octet-stream"'
            
            value_str = _ast_to_signature_string(node.value) if hasattr(node, 'value') else ''
...
def _extract_endpoint_from_client_request(call_node: ast.Call) -> Optional[str]:
    ...
    if (isinstance(call_node.func, ast.Attribute) and
        ...
        call_node.func.value.attr == 'client' and
        call_node.func.attr == 'request'):
...
def _extract_endpoint_from_assignment(value_node: ast.AST) -> Optional[str]:
    ...
    # Handle format calls like "/rest/ofscCore/v1/users/{}".format(user_id)
        ...
        if (isinstance(value_node.func, ast.Attribute) and 
            value_node.func.attr == 'format'):
...
def _infer_crud_endpoint(method_name: str) -> Optional[str]:
    ...
    patterns = [
        ...
        (r'^get_([a-z_]+)$', r'/rest/ofscMetadata/v1/\1s/{label}'),  # get_user -> /users/{label}
        
        # CREATE operations  
        (r'^create_([a-z_]+)$', r'/rest/ofscCore/v1/\1s'),  # create_user -> /users
        (r'^create_or_replace_([a-z_]+)$', r'/rest/ofscMetadata/v1/\1s/{label}'),  # create_or_replace_user -> /users/{label}
        
        # UPDATE operations
        (r'^update_([a-z_]+)$', r'/rest/ofscCore/v1/\1s/{id}'),  # update_user -> /users/{id}
        
        # DELETE operations
        (r'^delete_([a-z_]+)$', r'/rest/ofscCore/v1/\1s/{id}'),  # delete_user -> /users/{id}
    ]
...
def _extract_endpoint_from_docstring(method_node: ast.AsyncFunctionDef) -> Optional[str]:
...
def find_method_for_endpoint(method_mapping: Dict[str, List[Dict[str, Any]]], 
...
def _find_best_method_match(candidates: List[Tuple[str, Dict[str, Any], str]], 
                           http_method: str, endpoint_path: str) -> Optional[Tuple[str, Dict[str, Any], str]]:
    ...
    for endpoint_url, method_info, match_type in candidates:
        ...
        if http_method == 'GET':
            ...
            if method_name.startswith(('create_', 'add_', 'start_', 'complete_', 'cancel_', 'move_')):
                score += 100
...
def generate_method_signature(method_mapping: Dict[str, Dict[str, Any]], 
                            endpoint_method: str, endpoint_path: str, response_schema: str = "") -> str:
    """Generate a method signature for the endpoint if implemented."""
    
    # Use the HTTP method-aware matching to find the correct method
    method_name = find_method_for_endpoint(method_mapping, endpoint_method, endpoint_path)
    if not method_name:
        return ""
    ...
    if 'return_annotation' in method_info and method_info['return_annotation']:
        ...
        return_type = map_schema_to_model_name(response_schema) if response_schema else "ResponseModel"
    
    signature = f"async def {method_name}({', '.join(['self'] + params)}) -> {return_type}"
    return signature


def _infer_basic_parameters(endpoint_method: str, endpoint_path: str, method_name: str) -> List[str]:
...
def extract_endpoints_from_swagger(swagger_file: str) -> List[EndpointInfo]:
    ...
    for path, methods in paths.items():
        for method_name, method_data in methods.items():
            ...
            signature = generate_method_signature(method_mapping, method, path, response_schema)
