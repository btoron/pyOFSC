...
from tests.fixtures.endpoints_registry import ENDPOINTS, get_endpoint_by_id
from tests.fixtures.models_registry import (
    get_model_by_name,
    get_model_inheritance_chain,
    ModelInfo,
    ModelProperty,
)
...
def analyze_model_structure(
    model: ModelInfo, max_depth: int = 2, current_depth: int = 0
) -> str:
    ...
    # Model header
    mapped_info = (
        f" -> {model.mapped_pydantic_class}"
        if model.mapped_pydantic_class
        else " (no Pydantic mapping)"
    )
        ...
        for prop in model.properties:
            ...
            if prop.ref and current_depth < max_depth - 1:
                ...
                if nested_model:
                    ...
                    nested_structure = analyze_model_structure(
                        nested_model, max_depth, current_depth + 2
                    )
...
def analyze_endpoint_model_mapping(endpoint_id: int) -> bool:
    ...
    if response_model.mapped_pydantic_class:
        ...
        print("   ❌ No Pydantic mapping found")
        print(
            f"   💡 Suggestion: Implement Pydantic model for '{endpoint.response_schema}'"
        )
    ...
    if len(response_model.used_in_endpoints) > 1:
        print("   Other endpoints using this model:")
    ...
    if response_model.nested_models:
        ...
        for nested_name in response_model.nested_models:
            ...
            if nested_model:
                ...
                pydantic_info = (
                    f" -> {nested_model.mapped_pydantic_class}"
                    if nested_model.mapped_pydantic_class
                    else ""
                )
    ...
    if not response_model.mapped_pydantic_class:
        print(
            f"   {implementation_step}. Create Pydantic model for '{endpoint.response_schema}'"
        )
        ...
        if response_model.inheritance:
            print(
                f"      Inheritance: Consider extending {', '.join(response_model.inheritance)}"
            )
        implementation_step += 1

    unmapped_nested = [
        name
        for name in response_model.nested_models
    ...
    if response_model.mapped_pydantic_class and endpoint.implemented_in:
        ...
        print(
            f"      Run: pytest tests/models/test_endpoints_{response_model.module}.py -v"
        )
        ...
        if not endpoint.implemented_in:
            print("      Missing: API method implementation")
        if not response_model.mapped_pydantic_class:
            print("      Missing: Pydantic model mapping")
...
def list_endpoints_summary():
    ...
    print("=" * 110)
    print(
        f"{'ID':<4} {'Method':<7} {'Module':<12} {'Response Schema':<25} {'Pydantic':<15} {'Impl':<6} {'Path'}"
    )
    print("-" * 110)
    ...
    for endpoint in ENDPOINTS:
        ...
        print(
            f"{endpoint.id:<4} {endpoint.method:<7} {endpoint.module:<12} {schema_name:<25} {pydantic_status:<15} {impl_status:<6} {path}"
        )
...
Examples:
  ...
  %(prog)s --list-endpoints | grep "❌"  # Show only unmapped endpoints
    ...
    group.add_argument(
        "endpoint_id", nargs="?", type=int, help="ID of the endpoint to analyze"
    )
    group.add_argument(
        "--list-endpoints",
        action="store_true",
