...
def parse_endpoints_md(file_path: str) -> List[EndpointInfo]:
    ...
    for line in content.split('\n'):
        ...
        if match:
            ...
            is_implemented = bool(implemented_in and 
                                implemented_in != "" and 
                                implemented_in.upper() != "DEPRECATED" and
                                "v3.0.0-dev" in implemented_in)
...
def analyze_coverage():
    ...
    coverage_percentage = len(with_responses) / len(implemented_get_endpoints) * 100 if implemented_get_endpoints else 0
    
    # Statistics
    print("📊 COVERAGE STATISTICS")
    print("=" * 50)
    print(f"Total GET endpoints: {len(get_endpoints)}")
    print(f"Implemented GET endpoints: {len(implemented_get_endpoints)} ({len(implemented_get_endpoints)/len(get_endpoints)*100:.1f}%)")
    print(f"Implemented GET endpoints with saved responses: {len(with_responses)} ({coverage_percentage:.1f}%)")
    print(f"Implemented GET endpoints missing saved responses: {len(without_responses)}")
    print()
    ...
    for module, stats in sorted(modules.items()):
        ...
        print(f"{status} {module}: {stats['with_responses']}/{stats['total']} ({coverage:.1f}%)")
    print()
    ...
    endpoints_with_responses = [ep for ep in implemented_get_endpoints if ep.id in saved_response_ids]
    endpoints_with_responses.sort(key=lambda x: x.id)
    
    for ep in endpoints_with_responses:
        # Find response files for this endpoint
        response_files = []
        for filename in os.listdir(response_dir) if os.path.exists(response_dir) else []:
    ...
    endpoints_without_responses = [ep for ep in implemented_get_endpoints if ep.id not in saved_response_ids]
    ...
    for ep in endpoints_without_responses:
        ...
        if ep.signature:
            print(f"       Signature: {ep.signature[:80]}{'...' if len(ep.signature) > 80 else ''}")
        print()
    ...
    if without_responses:
        ...
        for module, eps in sorted(by_module.items()):
            ...
            for ep in sorted(eps, key=lambda x: x.id):
                ...
                priority = "🔴 HIGH" if any(keyword in ep.path.lower() for keyword in ['activities', 'resources', 'users']) else \
                          "🟡 MED" if any(keyword in ep.path.lower() for keyword in ['properties', 'types', 'categories']) else \
                          "🟢 LOW"
    ...
    print()
    print("=" * 80)
    print(f"Analysis complete. Coverage: {coverage_percentage:.1f}% ({len(with_responses)}/{len(implemented_get_endpoints)})")
    print("=" * 80)
