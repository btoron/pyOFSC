...
class ParameterGenerator:
    ...
    BOUNDARIES = {
        "string": {"min_length": 1, "max_length": 255, "max_text_length": 4000},
        "integer": {"min": 0, "max": 2147483647},
        "duration": {
            "min": 1,
            "max": 1440,  # 24 hours in minutes
        },
        "array": {"min_items": 0, "max_items": 50},
        "date": {"min": datetime(2000, 1, 1), "max": datetime(2099, 12, 31)},
    }

    @classmethod
    def generate_string_boundaries(
        cls, field_name: str, nullable: bool = False, max_length: int = 255
    ) -> List[tuple]:
    ...
    @classmethod
    def generate_date_boundaries(
        cls, field_name: str, allow_past: bool = True, allow_future: bool = True
    ) -> List[tuple]:
        ...
        if allow_past:
            ...
            cases.append(
                (f"{field_name}_past", (now - timedelta(days=1)).isoformat(), False)
            )
        ...
        if allow_future:
            ...
            cases.append(
                (f"{field_name}_future", (now + timedelta(days=1)).isoformat(), False)
            )

        return cases

    @classmethod
    def generate_array_boundaries(
        cls,
        field_name: str,
        ...
        cases = [
            ...
            (
                f"{field_name}_min_items",
                [item_generator() for _ in range(min_items)],
                True,
            ),
            (
                f"{field_name}_max_items",
                [item_generator() for _ in range(max_items)],
                True,
            ),
        ]
        ...
        if max_items < 1000:  # Prevent huge arrays
            cases.append(
                (
                    f"{field_name}_over_max",
                    [item_generator() for _ in range(max_items + 1)],
                    False,
                )
            )
    ...
    def generate_activity_params(scenario: str = "basic") -> Dict[str, Any]:
        ...
        scenarios = {
            ...
            "detailed": {
                ...
                "inventory": {
                    "parts": [
                        {"id": f"PART{i}", "quantity": random.randint(1, 5)}
