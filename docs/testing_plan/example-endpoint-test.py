...
class TestActivitiesEndpoint(APITestBase):
    ...
    def activity_test_suite(self) -> EndpointTestSuite:
        ...
        for (
            ...
            value,
            should_pass,
        ) in ParameterGenerator.generate_string_boundaries("name"):
            params = {
                "name": value,
                "activityType": "service",
                "duration": 60,
        ...
        for (
            ...
            value,
            should_pass,
        ) in ParameterGenerator.generate_numeric_boundaries(
        ...
        test_cases.extend(
            [
                ParameterTestCase(
                    ...
                    parameters={"activityType": "service", "duration": 60},
                    expected_status=400,
    ...
    @pytest.mark.parametrize(
        "test_case", lambda: activity_test_suite.test_cases, ids=lambda tc: tc.test_id
    )
    async def test_create_activity_comprehensive(
        self,
        client,
        ...
        async with rate_limiter(self.ENDPOINT_NAME):
            if test_case.expected_status == 200:
                ...
                try:
                    ...
                    if test_case.expected_response:
                        for (
                            field,
                            expected_value,
                        ) in test_case.expected_response.items():
                    ...
                    if test_case.should_cleanup:
                        created_resources.append(
                            {"type": "activity", "id": response.id, "client": client}
                        )
                ...
                await self.assert_error_response(
                    exc_info.value, test_case.expected_status, test_case.expected_error
                )

                print(
                    f"✓ {test_case.test_id}: Correctly rejected with: {exc_info.value}"
                )

    @pytest.mark.integration
    async def test_activity_crud_operations(
        self, client, created_resources, auto_cleanup
    ):
        ...
        created_resources.append(
            {"type": "activity", "id": created_activity.id, "client": client}
        )
        ...
        update_request = UpdateActivityRequest(
            duration=120, notes="Updated via API test"
        )

        updated_activity = await client.update_activity(
            created_activity.id, update_request
        )
    ...
    async def test_activity_pagination_complete(self, client):
        ...
        while True:
            response = await client.search_activities(
                limit=page_size, offset=offset, sortBy="id", sortOrder="asc"
            )
        ...
        assert len(activity_ids) == len(set(activity_ids)), (
            "Duplicate activities in pagination"
        )
    ...
    async def test_activity_concurrent_operations(self, client, rate_limiter):
        ...
        async def create_activity(index: int):
            async with rate_limiter(self.ENDPOINT_NAME):
                request = CreateActivityRequest(
                    name=f"Concurrent Test {index}", activityType="service", duration=30
                )
        ...
        assert duration > 1.0, (
            "Concurrent operations too fast, rate limiting may not be working"
        )
        ...
        print(
            f"✓ Concurrent operations: {len(successful)} succeeded, {len(failed)} failed"
        )
