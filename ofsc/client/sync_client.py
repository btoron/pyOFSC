"""Sync OFSC client implementation using httpx.Client."""
...
class OFSC(BaseOFSClient):
    """Synchronous OFSC client using httpx.Client."""
    ...
    def __init__(
        ...
        """Initialize the sync OFSC client.
    ...
    def _create_client(self) -> httpx.Client:
        """Create a sync httpx client with proper configuration."""
        return httpx.Client(
    ...
    def __enter__(self):
        ...
        logging.info(f"Sync OFSC client connected to {self.base_url}")
        return self
    ...
    def close(self):
        """Close the sync HTTP client and clean up resources."""
        if isinstance(self._client, httpx.Client) and not self._client.is_closed:
            self._client.close()
            logging.info("Sync OFSC client closed")
    ...
    def client(self) -> httpx.Client:
        ...
        if not isinstance(self._client, httpx.Client):
            raise RuntimeError("Expected httpx.Client instance.")
        return self._client
