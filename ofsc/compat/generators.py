...
def create_sync_method(api_name: str, method_name: str, method_config: Dict[str, Any]) -> Callable:
    ...
    def sync_method(self, *args, **kwargs):
        ...
        async def _async_call():
            async with AsyncOFSC(**self._config) as client:
                ...
                if api is None:
                    raise NotImplementedError(f"{api_name} API not yet implemented in v3.0")
                
                method = getattr(api, method_name)
                if method is None:
                    raise AttributeError(f"Method {method_name} not found in {api_name} API")
...
def create_deprecated_method(original_name: str, new_api: str, new_method: str) -> Callable:
    ...
    deprecated_method.__doc__ = f"""DEPRECATED: Use client.{new_api}.{new_method}() instead."""
