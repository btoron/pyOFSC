...
class ofscTest(unittest.TestCase):
    ...
    def test_get_properties(self):
        ...
        self.assertEqual(response['totalResults'], 465)
    ...
    def test_get_file_property_01(self):
        ...
        instance = self.instance
        logger = self.logger
        raw_response = instance.get_file_property(activityId=3954865, label="csign", mediaType="*/*", response_type=FULL_RESPONSE)
        logging.debug(self.pp.pformat(raw_response.json()))
        response = raw_response.json()
        logger.debug(self.pp.pformat(response))
    ...
    def test_get_file_property_02(self):
        ...
        instance = self.instance
        logger = self.logger
        metadata_response = instance.get_file_property(activityId=3954865, label="csign", mediaType="*/*", response_type=FULL_RESPONSE)
        logging.debug(self.pp.pformat(metadata_response.json()))
        response = metadata_response.json()
        raw_response = instance.get_file_property(activityId=3954865, label="csign", mediaType="image/png", response_type=FULL_RESPONSE)
