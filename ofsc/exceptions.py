...
def create_exception_from_response(
    response: "httpx.Response", message: Optional[str] = None
) -> OFSException:
        ...
        if response.content:
            ...
            if isinstance(data, dict):
                message = message or data.get(
                    "message", data.get("error", f"HTTP {status_code}")
                )
    ...
    return exception_class(
        message, status_code=status_code, response=response, **error_details
    )
