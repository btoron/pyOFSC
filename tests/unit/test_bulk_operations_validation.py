...
class TestBulkOperationModelsValidation:
    ...
    def test_get_activity_request_validation(self):
        ...
        request = GetActivityRequest(
            resources=["TECH001"], dateFrom=date(2024, 1, 1), dateTo=date(2024, 1, 31)
        )
    ...
    def test_activity_keys_validation(self):
        ...
        keys = ActivityKeys(
            activityId=12345, apptNumber="APPT001", customerNumber="CUST001"
        )
    ...
    def test_bulk_update_request_validation(self):
        ...
        request = BulkUpdateRequest(
            activities=[
                BulkUpdateActivityItem(activityId=12345, status="started"),
                BulkUpdateActivityItem(activityId=67890, status="completed"),
            ],
            updateParameters=BulkUpdateParameters(
                identifyActivityBy="activityId", fallbackResource="TECH999"
            ),
        )
    ...
    def test_bulk_update_request_required_fields(self):
        ...
        request = BulkUpdateRequest(
            activities=[], updateParameters=BulkUpdateParameters()
        )
    ...
    def test_bulk_update_error_validation(self):
        ...
        error = BulkUpdateError(
            errorDetail="Invalid status transition", operation="update_status"
        )
    ...
    def test_bulk_update_result_validation(self):
        ...
        result = BulkUpdateResult(
            activityKeys=ActivityKeys(activityId=11111, apptNumber="APPT123"),
            errors=[BulkUpdateError(errorDetail="Some error", operation="update")],
            warnings=[BulkUpdateWarning(code=200, message=300)],
        )
    ...
    def test_bulk_update_response_validation(self):
        ...
        response = BulkUpdateResponse(
            results=[
                ...
                BulkUpdateResult(
                    activityKeys=ActivityKeys(activityId=67890), errors=[], warnings=[]
                ),
            ]
        )
    ...
    def test_complex_bulk_update_scenario(self):
        ...
        complex_request = BulkUpdateRequest(
            activities=[
                BulkUpdateActivityItem(
                    activityId=1001, status="started", timeSlot="09:00-11:00"
                ),
                BulkUpdateActivityItem(
                    activityId=1002,
                    status="completed",
    ...
    def test_bulk_operation_model_serialization(self):
        ...
        request = BulkUpdateRequest(
            activities=[BulkUpdateActivityItem(activityId=12345, status="started")],
