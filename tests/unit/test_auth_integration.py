...
class TestAuthenticationIntegration:
    ...
    def test_client_creation_missing_credentials(self):
        ...
        try:
            with pytest.raises(
                OFSConfigurationException, match="instance must be provided"
            ):
                OFSC(client_id="test", client_secret="test")  # Missing instance

            with pytest.raises(
                OFSConfigurationException, match="client_id must be provided"
            ):
                OFSC(instance="test", client_secret="test")  # Missing client_id

            with pytest.raises(
                OFSConfigurationException, match="client_secret must be provided"
            ):
    ...
    def test_base_url_generation(self):
        ...
        for instance, expected_url in test_cases:
            client = OFSC(instance=instance, client_id="test", client_secret="test")
    ...
    def test_oauth2_error_handling_with_new_exceptions(self):
        ...
        with pytest.raises(
            (OFSAuthenticationException, OFSConnectionException)
        ) as exc_info:
            oauth2_auth._request_token()

        # Verify exception is from our hierarchy
        assert isinstance(
            exc_info.value, (OFSAuthenticationException, OFSConnectionException)
        )

    def test_context_manager_support(self):
        """Test that clients work as context managers."""
        async_client = OFSC(instance="test", client_id="test", client_secret="test")
    ...
    def test_auth_headers_generation(self):
        ...
        # Test Basic Auth
        basic_client = OFSC(
            instance="test", client_id="user", client_secret="pass", use_token=False
        )
        ...
        oauth_client = OFSC(
            instance="test", client_id="user", client_secret="pass", use_token=True
        )
