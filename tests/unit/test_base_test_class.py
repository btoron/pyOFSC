...
from tests.utils.base_test import (
    BaseOFSCTest,
    TestResourceTracker,
    PerformanceTracker,
    TestNameGenerator,
...
class TestNameGeneratorUnit:
    ...
    def test_generate_label_with_length_constraint(self):
        generator = TestNameGenerator(
            "VeryLongTestClassName", "very_long_test_method_name"
        )
...
class TestBaseOFSCTestClass(BaseOFSCTest):
    ...
    async def test_expect_exception_context_manager(self):
        ...
        with pytest.raises(
            AssertionError, match="Expected ValueError but no exception"
        ):
        ...
        with pytest.raises(
            AssertionError, match="Expected ValueError but got RuntimeError"
        ):
            async with self.expect_exception(ValueError):
        ...
        with pytest.raises(AssertionError, match="does not contain"):
            async with self.expect_exception(
                ValueError, message_contains="expected text"
            ):
