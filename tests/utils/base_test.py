...
class BaseOFSCTest:
    ...
    def assert_performance_within_limits(self, operation_name: str, max_duration: float):
    ...
    def assert_pydantic_model_valid(
        ...
        try:
            ...
            self._add_error_context('model_validation_error', {
    ...
    def set_endpoint_context(self, endpoint_id: Optional[int] = None, endpoint_info: Optional[EndpointInfo] = None):
    ...
    def add_request_context(self, method: str, url: str, **kwargs):
        ...
        request_info = {
            ...
            'headers': {k: v for k, v in kwargs.get('headers', {}).items() 
                      if k.lower() not in ['authorization']},  # Don't log auth headers
        }
        self.test_context['requests_made'].append(request_info)
    
    def add_response_context(self, response: Union[Response, Dict[str, Any]], status_code: Optional[int] = None):
        ...
        if isinstance(response, Response):
            ...
            response_info = {
                ...
                'data_keys': list(response.keys()) if isinstance(response, dict) else 'non-dict',
    ...
    def assert_response_time_acceptable(self, operation_name: str, max_seconds: float = 5.0):
    ...
    @asynccontextmanager
    async def api_call_context(self, endpoint_id: Optional[int] = None, operation_name: Optional[str] = None):
    ...
    @asynccontextmanager
    async def expect_exception(self, exception_class: Type[Exception], message_contains: Optional[str] = None):
