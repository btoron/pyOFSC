...
class SwaggerSchemaValidator:
    ...
    def validate_model_against_schema(
        self, model_class: Type[BaseModel], schema_name: str
    ) -> ValidationResult:
        """
        ...
        structural_result = self._validate_structure(
            model_class, schema_def, schema_name
        )
        ...
        data_result = self._validate_data_compatibility(
            model_class, schema_def, schema_name
        )
    ...
    def _validate_structure(
        self, model_class: Type[BaseModel], schema_def: Dict[str, Any], schema_name: str
    ) -> Dict[str, List[str]]:
        ...
        if missing_required:
            errors.append(
                f"Model missing required fields from schema: {sorted(missing_required)}"
            )
        ...
        if extra_required:
            warnings.append(
                f"Model has required fields not in schema: {sorted(extra_required)}"
            )
        ...
        for field_name, schema_property in schema_properties.items():
            if field_name in model_fields:
                ...
                if field_name in schema_required:
                    errors.append(
                        f"Required schema field '{field_name}' not found in model"
                    )
                else:
                    warnings.append(
                        f"Optional schema field '{field_name}' not found in model"
                    )
        ...
        if extra_model_fields:
            warnings.append(
                f"Model has fields not in schema: {sorted(extra_model_fields)}"
            )

        return {"errors": errors, "warnings": warnings}

    def _validate_field_compatibility(
        self, field_name: str, model_field: Any, schema_property: Dict[str, Any]
        ...
        model_type = (
            ...
            if hasattr(model_field, "annotation")
            else "unknown"
        )
    ...
    def _are_types_compatible(
        self, model_field: Any, schema_property: Dict[str, Any]
    ) -> bool:
    ...
    def _generate_data_from_schema_properties(
        self, schema_def: Dict[str, Any], schema_name: str, visited_schemas: Set[str]
    ) -> Dict[str, Any]:
    ...
    def _generate_value_for_property(
        self, prop_def: Dict[str, Any], prop_name: str, visited_schemas: Set[str]
    ) -> Any:
    ...
    def _generate_array_value(
        self, prop_def: Dict[str, Any], visited_schemas: Set[str]
    ) -> List[Any]:
