...
class TestTemplateGenerator:
    ...
    def find_resource_endpoints(self, resource_name: str) -> List[EndpointInfo]:
        ...
        for endpoint in self.endpoints:
            ...
            if (resource_name in path_clean or 
                resource_name in summary_clean or
                resource_name in endpoint.path.lower()):
    ...
    def generate_test_file(self, resource_name: str, output_path: Optional[str] = None) -> str:
        ...
        test_content = self._generate_test_content(resource_name, endpoints, crud_pattern)
    ...
    def _generate_test_content(
        ...
        sections.append(self._generate_header(resource_name, primary_module, model_names))
    ...
    def _generate_header(self, resource_name: str, module: str, model_names: Set[str]) -> str:
...
from tests.utils.factories import (
    ...
    def _generate_test_class_header(self, resource_name: str) -> str:
        ...
        class_name = ''.join(word.capitalize() for word in resource_name.replace('-', '_').split('_'))
    ...
    def _generate_setup_methods(self, resource_name: str, crud_pattern: Optional[CRUDPattern]) -> str:
    ...
    def _generate_create_test(self, crud_pattern: CRUDPattern) -> str:
        ...
        create_endpoint = crud_pattern.endpoints.get('POST') or crud_pattern.endpoints.get('PUT')
    ...
    def _generate_update_test(self, crud_pattern: CRUDPattern) -> str:
        ...
        update_endpoint = crud_pattern.endpoints.get('PUT') or crud_pattern.endpoints.get('PATCH')
    ...
    def _generate_parameter_boundary_tests(self, endpoint: EndpointInfo, param: EndpointParameter) -> List[str]:
        ...
        {f"min_length_value = 'a' * {param.min_length}" if param.min_length else "# No minimum length constraint"}
        {f"max_length_value = 'a' * {param.max_length}" if param.max_length else "# No maximum length constraint"}
        ...
        {f"min_value = {param.minimum}" if param.minimum is not None else "# No minimum value constraint"}
        {f"max_value = {param.maximum}" if param.maximum is not None else "# No maximum value constraint"}
        ...
        async with self.expect_exception(OFSValidationException):
            ...
            pass
        """ if param.minimum is not None else ""}
        
        {f"""
        async with self.expect_exception(OFSValidationException):
            # TODO: Add API call with value above maximum: {param.maximum + 1}
            pass
        """ if param.maximum is not None else ""}''')
    ...
    def _generate_search_filter_tests(self, get_endpoints: List[EndpointInfo]) -> List[str]:
        """Generate search and filter tests for GET endpoints."""
        tests = []
        
        for endpoint in get_endpoints:
            # Check if endpoint supports search/filter parameters
            search_params = [p for p in endpoint.optional_parameters 
            ...
            filter_params = [p for p in endpoint.optional_parameters
    ...
    def _generate_search_test(self, endpoint: EndpointInfo, search_params: List[EndpointParameter]) -> str:
    ...
    def _generate_filter_test(self, endpoint: EndpointInfo, filter_params: List[EndpointParameter]) -> str:
    ...
    def _generate_pagination_tests(self, get_endpoints: List[EndpointInfo]) -> List[str]:
        """Generate pagination tests for GET endpoints."""
        tests = []
        
        for endpoint in get_endpoints:
            # Check if endpoint supports pagination
            has_offset = any(p.name.lower() == 'offset' for p in endpoint.optional_parameters)
            has_limit = any(p.name.lower() == 'limit' for p in endpoint.optional_parameters)
    ...
    def _generate_bulk_performance_test(self, endpoint: EndpointInfo) -> str:
        ...
        method_name = self._generate_test_method_name(endpoint, suffix='bulk_performance')
    ...
    def _generate_test_method_name(self, endpoint: EndpointInfo, suffix: str = None) -> str:
        """Generate a test method name for an endpoint."""
        # Clean up the path to create a method name
        path_parts = endpoint.path.split('/')
        relevant_parts = [part for part in path_parts 
                         if part and not part.startswith('{') and 'rest' not in part and 'ofsc' not in part and 'v1' not in part and 'v2' not in part]
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Generate comprehensive test files for OFSC API endpoints')
    parser.add_argument('resource_name', help='Name of the resource to generate tests for (e.g., "properties", "activities")')
    parser.add_argument('--output', '-o', help='Output file path (default: auto-generated)')
    parser.add_argument('--list-resources', '-l', action='store_true', help='List available resources')
