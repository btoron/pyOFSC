...
class EndpointSchemaTestBase:
    ...
    def setup_method(self):
        ...
        swagger_file = (
            Path(__file__).parent.parent.parent / "response_examples" / "swagger.json"
        )
    ...
    def get_endpoints_for_module(self, module_name: str) -> List[EndpointInfo]:
        ...
        return [
            ...
            if (
                endpoint.method == "GET"
                and endpoint.response_schema
                and endpoint.response_schema.strip()
            )
        ]

    def validate_endpoint_schema_compatibility(
    ...
    def _perform_endpoint_validation(self, endpoint: EndpointInfo) -> ValidationResult:
        ...
        return self.schema_validator.validate_model_against_schema(
            model_class, schema_name
        )

    def _assert_validation_success(
        self, endpoint: EndpointInfo, result: ValidationResult
    ):
        ...
        error_parts.extend(
            [
                ...
                "  3. Ensure field names, types, and required/optional status match",
                "  4. If the model name doesn't match schema name, add override to SCHEMA_TO_MODEL_OVERRIDES",
            ]
        )
    ...
    def assert_field_compatibility(
        self, model_field: Any, schema_property: Dict[str, Any], field_name: str
    ):
        """
    ...
    def assert_required_fields_match(
        self, model_class: Type[BaseModel], schema_def: Dict[str, Any]
    ):
        """
    ...
    def generate_module_coverage_report(self, module_name: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": validated_count / total_endpoints
            if total_endpoints > 0
            else 0,
            "successful_validations": successful_validations,
            "failed_validations": failed_validations,
            "missing_models": list(set(missing_models)),
            "coverage_percentage": (validated_count / total_endpoints * 100)
            if total_endpoints > 0
            else 0,
        }
    ...
    def generate_test_id(self, endpoint: EndpointInfo) -> str:
        ...
        clean_path = (
            endpoint.path.replace("/rest/ofsc", "")
            .replace("/v1", "")
            .replace("/", "_")
            .replace("{", "")
            .replace("}", "")
