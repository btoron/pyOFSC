...
class TestCapacityEndpointsSchemaCompatibility(EndpointSchemaTestBase):
    ...
    def test_capacity_schema_coverage(self):
        ...
        min_coverage_percentage = (
            0.0  # Allow 0% for now since models aren't implemented yet
        )
        ...
        if report["missing_models"]:
            print("  Missing models that need implementation:")
    ...
    def test_capacity_model_completeness(self):
        ...
        if capacity_related_orphans:
            ...
            for model in sorted(capacity_related_orphans):
                print(f"  - {model}")
            print("These models might need:")
            print("  1. Corresponding swagger schema definitions")
            print("  2. To be added to SCHEMA_TO_MODEL_OVERRIDES mapping")
            print("  3. Review if they're still needed")
    ...
    def test_capacity_quota_schemas_coverage(self):
        ...
        coverage_ratio = (
            len(found_schemas) / len(quota_schemas) if quota_schemas else 1.0
        )
    ...
    def test_capacity_booking_schemas_coverage(self):
        ...
        coverage_ratio = (
            len(found_schemas) / len(booking_schemas) if booking_schemas else 1.0
        )
        ...
        if len(found_schemas) == 0 and len(booking_schemas) > 0:
            ...
            print(
                "        functionality is not yet implemented or uses different naming."
            )
...
if __name__ == "__main__":
    ...
    print(
        f"Found {len(test_instance.capacity_get_endpoints)} capacity GET endpoints with schemas"
    )
