...
class TestCoreModelsValidation:
    ...
    def test_activity_installed_inventory_list_response_validation(
            ...
            if "items" in data and isinstance(data["items"], list):
                if True:
                    ...
                    try:
                        ...
                        self._validate_activity_installed_inventory_list_response_fields(
                            model_instance, data
                        )
                        print(f"✅ Validated {filename} as list response")
                    except ValidationError as e:
                        pytest.fail(
                            f"ActivityInstalledInventoryListResponse validation failed for {filename}: {e}"
                        )
                    ...
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            print(
                                f"✅ Validated {filename} item {idx} with ActivityInstalledInventory"
                            )
                        except ValidationError as e:
                            pytest.fail(
                                f"ActivityInstalledInventory validation failed for {filename} item {idx}: {e}"
                            )
                else:
                    # For non-list models, validate individual items using the same model
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            model_instance = ActivityInstalledInventoryListResponse(
                                **item
                            )
                            self._validate_activity_installed_inventory_list_response_fields(
                                model_instance, item
                            ...
                            pytest.fail(
                                f"ActivityInstalledInventoryListResponse validation failed for {filename} item {idx}: {e}"
                            )
            else:
    ...
    def _validate_activity_installed_inventory_list_response_fields(
        self, model: ActivityInstalledInventoryListResponse, original_data: dict
    ):
            ...
            if "items" in data and isinstance(data["items"], list):
                if True:
                    ...
                    try:
                        ...
                        pytest.fail(
                            f"UserListResponse validation failed for {filename}: {e}"
                        )
                    ...
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            pytest.fail(
                                f"User validation failed for {filename} item {idx}: {e}"
                            )
                else:
                    # For non-list models, validate individual items using the same model
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            self._validate_user_list_response_fields(
                                model_instance, item
                            )
                            print(f"✅ Validated {filename} item {idx}")
                        except ValidationError as e:
                            pytest.fail(
                                f"UserListResponse validation failed for {filename} item {idx}: {e}"
                            )
            else:
                # Validate single response
                try:
    ...
    def _validate_user_list_response_fields(
        self, model: UserListResponse, original_data: dict
    ):
    ...
    def test_resource_work_schedule_item_list_response_validation(
        self, response_examples_path
    ):
            ...
            if "items" in data and isinstance(data["items"], list):
                if True:
                    ...
                    try:
                        ...
                        self._validate_resource_work_schedule_item_list_response_fields(
                            model_instance, data
                        )
                        print(f"✅ Validated {filename} as list response")
                    except ValidationError as e:
                        pytest.fail(
                            f"ResourceWorkScheduleItemListResponse validation failed for {filename}: {e}"
                        )
                    ...
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            print(
                                f"✅ Validated {filename} item {idx} with ResourceWorkScheduleItem"
                            )
                        except ValidationError as e:
                            pytest.fail(
                                f"ResourceWorkScheduleItem validation failed for {filename} item {idx}: {e}"
                            )
                else:
                    # For non-list models, validate individual items using the same model
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            model_instance = ResourceWorkScheduleItemListResponse(
                                **item
                            )
                            self._validate_resource_work_schedule_item_list_response_fields(
                                model_instance, item
                            ...
                            pytest.fail(
                                f"ResourceWorkScheduleItemListResponse validation failed for {filename} item {idx}: {e}"
                            )
            else:
                # Validate single response
                try:
    ...
    def _validate_resource_work_schedule_item_list_response_fields(
        self, model: ResourceWorkScheduleItemListResponse, original_data: dict
    ):
            ...
            if "items" in data and isinstance(data["items"], list):
                if True:
                    ...
                    try:
                        ...
                        self._validate_location_list_response_fields(
                            model_instance, data
                        )
                        print(f"✅ Validated {filename} as list response")
                    except ValidationError as e:
                        pytest.fail(
                            f"LocationListResponse validation failed for {filename}: {e}"
                        )
                    ...
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            pytest.fail(
                                f"Location validation failed for {filename} item {idx}: {e}"
                            )
                else:
                    # For non-list models, validate individual items using the same model
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            pytest.fail(
                                f"LocationListResponse validation failed for {filename} item {idx}: {e}"
                            )
            else:
                # Validate single response
                try:
    ...
    def _validate_location_list_response_fields(
        self, model: LocationListResponse, original_data: dict
    ):
            ...
            if "items" in data and isinstance(data["items"], list):
                if True:
                    ...
                    try:
                        ...
                        self._validate_last_known_position_list_response_fields(
                            model_instance, data
                        )
                        print(f"✅ Validated {filename} as list response")
                    except ValidationError as e:
                        pytest.fail(
                            f"LastKnownPositionListResponse validation failed for {filename}: {e}"
                        )
                    ...
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            print(
                                f"✅ Validated {filename} item {idx} with LastKnownPosition"
                            )
                        except ValidationError as e:
                            pytest.fail(
                                f"LastKnownPosition validation failed for {filename} item {idx}: {e}"
                            )
                else:
                    # For non-list models, validate individual items using the same model
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            pytest.fail(
                                f"LastKnownPositionListResponse validation failed for {filename} item {idx}: {e}"
                            )
            else:
                # Validate single response
                try:
    ...
    def _validate_last_known_position_list_response_fields(
        self, model: LastKnownPositionListResponse, original_data: dict
    ):
                ...
                if False:
                    ...
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            pytest.fail(
                                f"Activity validation failed for {filename} item {idx}: {e}"
                            )
            else:
                # Validate single response
                try:
                ...
                if False:
                    ...
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            pytest.fail(
                                f"Resource validation failed for {filename} item {idx}: {e}"
                            )
            else:
                # Validate single response
                try:
            ...
            if "items" in data and isinstance(data["items"], list):
                if True:
                    ...
                    try:
                        ...
                        self._validate_resource_inventory_list_response_fields(
                            model_instance, data
                        )
                        print(f"✅ Validated {filename} as list response")
                    except ValidationError as e:
                        pytest.fail(
                            f"ResourceInventoryListResponse validation failed for {filename}: {e}"
                        )
                    ...
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            print(
                                f"✅ Validated {filename} item {idx} with ResourceInventory"
                            )
                        except ValidationError as e:
                            pytest.fail(
                                f"ResourceInventory validation failed for {filename} item {idx}: {e}"
                            )
                else:
                    # For non-list models, validate individual items using the same model
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            pytest.fail(
                                f"ResourceInventoryListResponse validation failed for {filename} item {idx}: {e}"
                            )
            else:
                # Validate single response
                try:
    ...
    def _validate_resource_inventory_list_response_fields(
        self, model: ResourceInventoryListResponse, original_data: dict
    ):
            ...
            if "items" in data and isinstance(data["items"], list):
                if True:
                    ...
                    try:
                        ...
                        self._validate_resource_list_response_fields(
                            model_instance, data
                        )
                        print(f"✅ Validated {filename} as list response")
                    except ValidationError as e:
                        pytest.fail(
                            f"ResourceListResponse validation failed for {filename}: {e}"
                        )
                    ...
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            pytest.fail(
                                f"ResourceListResponse validation failed for {filename} item {idx}: {e}"
                            )
            else:
                # Validate single response
                try:
    ...
    def _validate_resource_list_response_fields(
        self, model: ResourceListResponse, original_data: dict
    ):
            ...
            if "items" in data and isinstance(data["items"], list):
                if False:
                    ...
                    try:
                        ...
                        self._validate_assigned_locations_response_fields(
                            model_instance, data
                        )
                        print(f"✅ Validated {filename} as list response")
                    except ValidationError as e:
                        pytest.fail(
                            f"AssignedLocationsResponse validation failed for {filename}: {e}"
                        )
                    ...
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            pytest.fail(
                                f"AssignedLocationsResponse validation failed for {filename} item {idx}: {e}"
                            )
            else:
                # Validate single response
                try:
    ...
    def _validate_assigned_locations_response_fields(
        self, model: AssignedLocationsResponse, original_data: dict
    ):
            ...
            if "items" in data and isinstance(data["items"], list):
                if True:
                    ...
                    try:
                        ...
                        self._validate_route_info_list_response_fields(
                            model_instance, data
                        )
                        print(f"✅ Validated {filename} as list response")
                    except ValidationError as e:
                        pytest.fail(
                            f"RouteInfoListResponse validation failed for {filename}: {e}"
                        )
                    ...
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            pytest.fail(
                                f"RouteInfoListResponse validation failed for {filename} item {idx}: {e}"
                            )
            else:
                # Validate single response
                try:
    ...
    def _validate_route_info_list_response_fields(
        self, model: RouteInfoListResponse, original_data: dict
    ):
                ...
                if False:
                    ...
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            pytest.fail(
                                f"Inventory validation failed for {filename} item {idx}: {e}"
                            )
            else:
                # Validate single response
                try:
    ...
    def test_activity_resource_preference_list_response_validation(
        self, response_examples_path
    ):
            ...
            if "items" in data and isinstance(data["items"], list):
                if True:
                    ...
                    try:
                        ...
                        self._validate_activity_resource_preference_list_response_fields(
                            model_instance, data
                        )
                        print(f"✅ Validated {filename} as list response")
                    except ValidationError as e:
                        pytest.fail(
                            f"ActivityResourcePreferenceListResponse validation failed for {filename}: {e}"
                        )
                    ...
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            print(
                                f"✅ Validated {filename} item {idx} with ActivityResourcePreference"
                            )
                        except ValidationError as e:
                            pytest.fail(
                                f"ActivityResourcePreference validation failed for {filename} item {idx}: {e}"
                            )
                else:
                    # For non-list models, validate individual items using the same model
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            model_instance = ActivityResourcePreferenceListResponse(
                                **item
                            )
                            self._validate_activity_resource_preference_list_response_fields(
                                model_instance, item
                            ...
                            pytest.fail(
                                f"ActivityResourcePreferenceListResponse validation failed for {filename} item {idx}: {e}"
                            )
            else:
                # Validate single response
                try:
    ...
    def _validate_activity_resource_preference_list_response_fields(
        self, model: ActivityResourcePreferenceListResponse, original_data: dict
    ):
    ...
    def test_activity_required_inventory_list_response_validation(
        self, response_examples_path
    ):
            ...
            if "items" in data and isinstance(data["items"], list):
                if True:
                    ...
                    try:
                        ...
                        self._validate_activity_required_inventory_list_response_fields(
                            model_instance, data
                        )
                        print(f"✅ Validated {filename} as list response")
                    except ValidationError as e:
                        pytest.fail(
                            f"ActivityRequiredInventoryListResponse validation failed for {filename}: {e}"
                        )
                    ...
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            print(
                                f"✅ Validated {filename} item {idx} with ActivityRequiredInventory"
                            )
                        except ValidationError as e:
                            pytest.fail(
                                f"ActivityRequiredInventory validation failed for {filename} item {idx}: {e}"
                            )
                else:
                    # For non-list models, validate individual items using the same model
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            model_instance = ActivityRequiredInventoryListResponse(
                                **item
                            )
                            self._validate_activity_required_inventory_list_response_fields(
                                model_instance, item
                            ...
                            pytest.fail(
                                f"ActivityRequiredInventoryListResponse validation failed for {filename} item {idx}: {e}"
                            )
            else:
                # Validate single response
                try:
    ...
    def _validate_activity_required_inventory_list_response_fields(
        self, model: ActivityRequiredInventoryListResponse, original_data: dict
    ):
            ...
            if "items" in data and isinstance(data["items"], list):
                if False:
                    ...
                    try:
                        ...
                        pytest.fail(
                            f"DailyExtractFiles validation failed for {filename}: {e}"
                        )
                    ...
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            self._validate_daily_extract_files_fields(
                                model_instance, item
                            )
                            print(f"✅ Validated {filename} item {idx}")
                        except ValidationError as e:
                            pytest.fail(
                                f"DailyExtractFiles validation failed for {filename} item {idx}: {e}"
                            )
            else:
                # Validate single response
                try:
    ...
    def _validate_daily_extract_files_fields(
        self, model: DailyExtractFiles, original_data: dict
    ):
            ...
            if "items" in data and isinstance(data["items"], list):
                if True:
                    ...
                    try:
                        ...
                        self._validate_activity_link_list_response_fields(
                            model_instance, data
                        )
                        print(f"✅ Validated {filename} as list response")
                    except ValidationError as e:
                        pytest.fail(
                            f"ActivityLinkListResponse validation failed for {filename}: {e}"
                        )
                    ...
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            print(
                                f"✅ Validated {filename} item {idx} with ActivityLink"
                            )
                        except ValidationError as e:
                            pytest.fail(
                                f"ActivityLink validation failed for {filename} item {idx}: {e}"
                            )
                else:
                    # For non-list models, validate individual items using the same model
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            pytest.fail(
                                f"ActivityLinkListResponse validation failed for {filename} item {idx}: {e}"
                            )
            else:
                # Validate single response
                try:
    ...
    def _validate_activity_link_list_response_fields(
        self, model: ActivityLinkListResponse, original_data: dict
    ):
    ...
    def test_activity_capacity_category_list_response_validation(
        self, response_examples_path
    ):
            ...
            if "items" in data and isinstance(data["items"], list):
                if True:
                    ...
                    try:
                        ...
                        self._validate_activity_capacity_category_list_response_fields(
                            model_instance, data
                        )
                        print(f"✅ Validated {filename} as list response")
                    except ValidationError as e:
                        pytest.fail(
                            f"ActivityCapacityCategoryListResponse validation failed for {filename}: {e}"
                        )
                    ...
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            print(
                                f"✅ Validated {filename} item {idx} with ActivityCapacityCategory"
                            )
                        except ValidationError as e:
                            pytest.fail(
                                f"ActivityCapacityCategory validation failed for {filename} item {idx}: {e}"
                            )
                else:
                    # For non-list models, validate individual items using the same model
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            model_instance = ActivityCapacityCategoryListResponse(
                                **item
                            )
                            self._validate_activity_capacity_category_list_response_fields(
                                model_instance, item
                            ...
                            pytest.fail(
                                f"ActivityCapacityCategoryListResponse validation failed for {filename} item {idx}: {e}"
                            )
            else:
                # Validate single response
                try:
    ...
    def _validate_activity_capacity_category_list_response_fields(
        self, model: ActivityCapacityCategoryListResponse, original_data: dict
    ):
            ...
            if "items" in data and isinstance(data["items"], list):
                if True:
                    ...
                    try:
                        ...
                        self._validate_resource_users_list_response_fields(
                            model_instance, data
                        )
                        print(f"✅ Validated {filename} as list response")
                    except ValidationError as e:
                        pytest.fail(
                            f"ResourceUsersListResponse validation failed for {filename}: {e}"
                        )
                    ...
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            pytest.fail(
                                f"ResourceUsersListResponse validation failed for {filename} item {idx}: {e}"
                            )
            else:
                # Validate single response
                try:
    ...
    def _validate_resource_users_list_response_fields(
        self, model: ResourceUsersListResponse, original_data: dict
    ):
    ...
    def test_activity_submitted_form_list_response_validation(
        self, response_examples_path
    ):
            ...
            if "items" in data and isinstance(data["items"], list):
                if True:
                    ...
                    try:
                        ...
                        self._validate_activity_submitted_form_list_response_fields(
                            model_instance, data
                        )
                        print(f"✅ Validated {filename} as list response")
                    except ValidationError as e:
                        pytest.fail(
                            f"ActivitySubmittedFormListResponse validation failed for {filename}: {e}"
                        )
                    ...
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            print(
                                f"✅ Validated {filename} item {idx} with ActivitySubmittedForm"
                            )
                        except ValidationError as e:
                            pytest.fail(
                                f"ActivitySubmittedForm validation failed for {filename} item {idx}: {e}"
                            )
                else:
                    # For non-list models, validate individual items using the same model
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            pytest.fail(
                                f"ActivitySubmittedFormListResponse validation failed for {filename} item {idx}: {e}"
                            )
            else:
                # Validate single response
                try:
    ...
    def _validate_activity_submitted_form_list_response_fields(
        self, model: ActivitySubmittedFormListResponse, original_data: dict
    ):
            ...
            if "items" in data and isinstance(data["items"], list):
                if True:
                    ...
                    try:
                        ...
                        self._validate_nearby_activity_list_response_fields(
                            model_instance, data
                        )
                        print(f"✅ Validated {filename} as list response")
                    except ValidationError as e:
                        pytest.fail(
                            f"NearbyActivityListResponse validation failed for {filename}: {e}"
                        )
                    ...
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            print(
                                f"✅ Validated {filename} item {idx} with NearbyActivity"
                            )
                        except ValidationError as e:
                            pytest.fail(
                                f"NearbyActivity validation failed for {filename} item {idx}: {e}"
                            )
                else:
                    # For non-list models, validate individual items using the same model
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            pytest.fail(
                                f"NearbyActivityListResponse validation failed for {filename} item {idx}: {e}"
                            )
            else:
                # Validate single response
                try:
    ...
    def _validate_nearby_activity_list_response_fields(
        self, model: NearbyActivityListResponse, original_data: dict
    ):
    ...
    def test_activity_deinstalled_inventory_list_response_validation(
        self, response_examples_path
    ):
        ...
        for filename in response_files:
            ...
            if "items" in data and isinstance(data["items"], list):
                if True:
                    # Validate the entire list response
                    try:
                        model_instance = ActivityDeinstalledInventoryListResponse(
                            **data
                        )
                        self._validate_activity_deinstalled_inventory_list_response_fields(
                            model_instance, data
                        )
                        print(f"✅ Validated {filename} as list response")
                    except ValidationError as e:
                        pytest.fail(
                            f"ActivityDeinstalledInventoryListResponse validation failed for {filename}: {e}"
                        )
                    ...
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            print(
                                f"✅ Validated {filename} item {idx} with ActivityDeinstalledInventory"
                            )
                        except ValidationError as e:
                            pytest.fail(
                                f"ActivityDeinstalledInventory validation failed for {filename} item {idx}: {e}"
                            )
                else:
                    # For non-list models, validate individual items using the same model
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            pytest.fail(
                                f"ActivityDeinstalledInventoryListResponse validation failed for {filename} item {idx}: {e}"
                            )
            else:
                # Validate single response
                try:
    ...
    def _validate_activity_deinstalled_inventory_list_response_fields(
        self, model: ActivityDeinstalledInventoryListResponse, original_data: dict
    ):
            ...
            if "items" in data and isinstance(data["items"], list):
                if True:
                    ...
                    try:
                        ...
                        self._validate_resource_work_skill_list_response_fields(
                            model_instance, data
                        )
                        print(f"✅ Validated {filename} as list response")
                    except ValidationError as e:
                        pytest.fail(
                            f"ResourceWorkSkillListResponse validation failed for {filename}: {e}"
                        )
                    ...
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            print(
                                f"✅ Validated {filename} item {idx} with ResourceWorkSkill"
                            )
                        except ValidationError as e:
                            pytest.fail(
                                f"ResourceWorkSkill validation failed for {filename} item {idx}: {e}"
                            )
                else:
                    # For non-list models, validate individual items using the same model
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            pytest.fail(
                                f"ResourceWorkSkillListResponse validation failed for {filename} item {idx}: {e}"
                            )
            else:
                # Validate single response
                try:
    ...
    def _validate_resource_work_skill_list_response_fields(
        self, model: ResourceWorkSkillListResponse, original_data: dict
    ):
            ...
            if "items" in data and isinstance(data["items"], list):
                if True:
                    ...
                    try:
                        ...
                        pytest.fail(
                            f"SubscriptionList validation failed for {filename}: {e}"
                        )
                    ...
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            print(
                                f"✅ Validated {filename} item {idx} with Subscription"
                            )
                        except ValidationError as e:
                            pytest.fail(
                                f"Subscription validation failed for {filename} item {idx}: {e}"
                            )
                else:
                    # For non-list models, validate individual items using the same model
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            self._validate_subscription_list_fields(
                                model_instance, item
                            )
                            print(f"✅ Validated {filename} item {idx}")
                        except ValidationError as e:
                            pytest.fail(
                                f"SubscriptionList validation failed for {filename} item {idx}: {e}"
                            )
            else:
                # Validate single response
                try:
    ...
    def _validate_subscription_list_fields(
        self, model: SubscriptionList, original_data: dict
    ):
            ...
            if "items" in data and isinstance(data["items"], list):
                if False:
                    ...
                    try:
                        ...
                        self._validate_daily_extract_folders_fields(
                            model_instance, data
                        )
                        print(f"✅ Validated {filename} as list response")
                    except ValidationError as e:
                        pytest.fail(
                            f"DailyExtractFolders validation failed for {filename}: {e}"
                        )
                    ...
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            pytest.fail(
                                f"DailyExtractFolders validation failed for {filename} item {idx}: {e}"
                            )
            else:
                # Validate single response
                try:
    ...
    def _validate_daily_extract_folders_fields(
        self, model: DailyExtractFolders, original_data: dict
    ):
        """Validate specific fields for DailyExtractFolders."""
        # Add model-specific field validations here
        pass  # Add specific field validations as needed

    def test_activity_customer_inventory_list_response_validation(
        self, response_examples_path
    ):
            ...
            if "items" in data and isinstance(data["items"], list):
                if True:
                    ...
                    try:
                        ...
                        self._validate_activity_customer_inventory_list_response_fields(
                            model_instance, data
                        )
                        print(f"✅ Validated {filename} as list response")
                    except ValidationError as e:
                        pytest.fail(
                            f"ActivityCustomerInventoryListResponse validation failed for {filename}: {e}"
                        )
                    ...
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            print(
                                f"✅ Validated {filename} item {idx} with ActivityCustomerInventory"
                            )
                        except ValidationError as e:
                            pytest.fail(
                                f"ActivityCustomerInventory validation failed for {filename} item {idx}: {e}"
                            )
                else:
                    # For non-list models, validate individual items using the same model
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            model_instance = ActivityCustomerInventoryListResponse(
                                **item
                            )
                            self._validate_activity_customer_inventory_list_response_fields(
                                model_instance, item
                            ...
                            pytest.fail(
                                f"ActivityCustomerInventoryListResponse validation failed for {filename} item {idx}: {e}"
                            )
            else:
                # Validate single response
                try:
    ...
    def _validate_activity_customer_inventory_list_response_fields(
        self, model: ActivityCustomerInventoryListResponse, original_data: dict
    ):
            ...
            if "items" in data and isinstance(data["items"], list):
                if True:
                    ...
                    try:
                        ...
                        self._validate_resource_work_zone_list_response_fields(
                            model_instance, data
                        )
                        print(f"✅ Validated {filename} as list response")
                    except ValidationError as e:
                        pytest.fail(
                            f"ResourceWorkZoneListResponse validation failed for {filename}: {e}"
                        )
                    ...
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            print(
                                f"✅ Validated {filename} item {idx} with ResourceWorkZone"
                            )
                        except ValidationError as e:
                            pytest.fail(
                                f"ResourceWorkZone validation failed for {filename} item {idx}: {e}"
                            )
                else:
                    # For non-list models, validate individual items using the same model
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            pytest.fail(
                                f"ResourceWorkZoneListResponse validation failed for {filename} item {idx}: {e}"
                            )
            else:
                # Validate single response
                try:
    ...
    def _validate_resource_work_zone_list_response_fields(
        self, model: ResourceWorkZoneListResponse, original_data: dict
    ):
