...
class TestCapacityModelsValidation:
            ...
            if "items" in data and isinstance(data["items"], list):
                if True:
                    ...
                    try:
                        ...
                        self._validate_capacity_area_list_response_fields(
                            model_instance, data
                        )
                        print(f"✅ Validated {filename} as list response")
                    except ValidationError as e:
                        pytest.fail(
                            f"CapacityAreaListResponse validation failed for {filename}: {e}"
                        )
                    ...
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            print(
                                f"✅ Validated {filename} item {idx} with CapacityArea"
                            )
                        except ValidationError as e:
                            pytest.fail(
                                f"CapacityArea validation failed for {filename} item {idx}: {e}"
                            )
                else:
                    ...
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            pytest.fail(
                                f"CapacityAreaListResponse validation failed for {filename} item {idx}: {e}"
                            )
            else:
    ...
    def _validate_capacity_area_list_response_fields(
        self, model: CapacityAreaListResponse, original_data: dict
    ):
            ...
            if "items" in data and isinstance(data["items"], list):
                if False:
                    ...
                    try:
                        ...
                        pytest.fail(
                            f"CapacityArea validation failed for {filename}: {e}"
                        )
    ...
    def _validate_capacity_area_fields(self, model: CapacityArea, original_data: dict):
        ...
        if hasattr(model, "type") and getattr(model, "type") is not None:
            assert getattr(model, "type") in ["group", "area"], (
                "Invalid enum value for type"
            )

    def test_capacity_area_time_slot_list_response_validation(
            ...
            if "items" in data and isinstance(data["items"], list):
                if True:
                    ...
                    try:
                        ...
                        self._validate_capacity_area_time_slot_list_response_fields(
                            model_instance, data
                        )
                        print(f"✅ Validated {filename} as list response")
                    except ValidationError as e:
                        pytest.fail(
                            f"CapacityAreaTimeSlotListResponse validation failed for {filename}: {e}"
                        )
                    ...
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            print(
                                f"✅ Validated {filename} item {idx} with CapacityAreaTimeSlot"
                            )
                        except ValidationError as e:
                            pytest.fail(
                                f"CapacityAreaTimeSlot validation failed for {filename} item {idx}: {e}"
                            )
                else:
                    # For non-list models, validate individual items using the same model
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            pytest.fail(
                                f"CapacityAreaTimeSlotListResponse validation failed for {filename} item {idx}: {e}"
                            )
            else:
                # Validate single response
                try:
    ...
    def _validate_capacity_area_time_slot_list_response_fields(
        self, model: CapacityAreaTimeSlotListResponse, original_data: dict
    ):
    ...
    def test_capacity_area_workzone_list_response_validation(
        self, response_examples_path
    ):
            ...
            if "items" in data and isinstance(data["items"], list):
                if True:
                    ...
                    try:
                        ...
                        self._validate_capacity_area_workzone_list_response_fields(
                            model_instance, data
                        )
                        print(f"✅ Validated {filename} as list response")
                    except ValidationError as e:
                        pytest.fail(
                            f"CapacityAreaWorkzoneListResponse validation failed for {filename}: {e}"
                        )
                    ...
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            print(
                                f"✅ Validated {filename} item {idx} with CapacityAreaWorkzone"
                            )
                        except ValidationError as e:
                            pytest.fail(
                                f"CapacityAreaWorkzone validation failed for {filename} item {idx}: {e}"
                            )
                else:
                    # For non-list models, validate individual items using the same model
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            pytest.fail(
                                f"CapacityAreaWorkzoneListResponse validation failed for {filename} item {idx}: {e}"
                            )
            else:
                # Validate single response
                try:
    ...
    def _validate_capacity_area_workzone_list_response_fields(
        self, model: CapacityAreaWorkzoneListResponse, original_data: dict
    ):
    ...
    def test_capacity_area_organization_list_response_validation(
        self, response_examples_path
    ):
            ...
            if "items" in data and isinstance(data["items"], list):
                if True:
                    ...
                    try:
                        ...
                        self._validate_capacity_area_organization_list_response_fields(
                            model_instance, data
                        )
                        print(f"✅ Validated {filename} as list response")
                    except ValidationError as e:
                        pytest.fail(
                            f"CapacityAreaOrganizationListResponse validation failed for {filename}: {e}"
                        )
                    ...
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            print(
                                f"✅ Validated {filename} item {idx} with CapacityAreaOrganization"
                            )
                        except ValidationError as e:
                            pytest.fail(
                                f"CapacityAreaOrganization validation failed for {filename} item {idx}: {e}"
                            )
                else:
                    # For non-list models, validate individual items using the same model
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            model_instance = CapacityAreaOrganizationListResponse(
                                **item
                            )
                            self._validate_capacity_area_organization_list_response_fields(
                                model_instance, item
                            ...
                            pytest.fail(
                                f"CapacityAreaOrganizationListResponse validation failed for {filename} item {idx}: {e}"
                            )
            else:
                # Validate single response
                try:
    ...
    def _validate_capacity_area_organization_list_response_fields(
        self, model: CapacityAreaOrganizationListResponse, original_data: dict
    ):
            ...
            if "items" in data and isinstance(data["items"], list):
                if True:
                    ...
                    try:
                        ...
                        self._validate_capacity_category_list_response_fields(
                            model_instance, data
                        )
                        print(f"✅ Validated {filename} as list response")
                    except ValidationError as e:
                        pytest.fail(
                            f"CapacityCategoryListResponse validation failed for {filename}: {e}"
                        )
                    ...
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            pytest.fail(
                                f"CapacityCategoryListResponse validation failed for {filename} item {idx}: {e}"
                            )
            else:
                # Validate single response
                try:
    ...
    def _validate_capacity_category_list_response_fields(
        self, model: CapacityCategoryListResponse, original_data: dict
    ):
            ...
            if "items" in data and isinstance(data["items"], list):
                if False:
                    ...
                    try:
                        ...
                        self._validate_capacity_category_response_fields(
                            model_instance, data
                        )
                        print(f"✅ Validated {filename} as list response")
                    except ValidationError as e:
                        pytest.fail(
                            f"CapacityCategoryResponse validation failed for {filename}: {e}"
                        )
                    ...
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            pytest.fail(
                                f"CapacityCategoryResponse validation failed for {filename} item {idx}: {e}"
                            )
            else:
                # Validate single response
                try:
    ...
    def _validate_capacity_category_response_fields(
        self, model: CapacityCategoryResponse, original_data: dict
    ):
        """Validate specific fields for CapacityCategoryResponse."""
        # Add model-specific field validations here
        pass  # Add specific field validations as needed

    def test_capacity_area_time_interval_list_response_validation(
        self, response_examples_path
    ):
            ...
            if "items" in data and isinstance(data["items"], list):
                if True:
                    ...
                    try:
                        ...
                        self._validate_capacity_area_time_interval_list_response_fields(
                            model_instance, data
                        )
                        print(f"✅ Validated {filename} as list response")
                    except ValidationError as e:
                        pytest.fail(
                            f"CapacityAreaTimeIntervalListResponse validation failed for {filename}: {e}"
                        )
                    ...
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            print(
                                f"✅ Validated {filename} item {idx} with CapacityAreaTimeInterval"
                            )
                        except ValidationError as e:
                            pytest.fail(
                                f"CapacityAreaTimeInterval validation failed for {filename} item {idx}: {e}"
                            )
                else:
                    # For non-list models, validate individual items using the same model
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            model_instance = CapacityAreaTimeIntervalListResponse(
                                **item
                            )
                            self._validate_capacity_area_time_interval_list_response_fields(
                                model_instance, item
                            ...
                            pytest.fail(
                                f"CapacityAreaTimeIntervalListResponse validation failed for {filename} item {idx}: {e}"
                            )
            else:
                # Validate single response
                try:
    ...
    def _validate_capacity_area_time_interval_list_response_fields(
        self, model: CapacityAreaTimeIntervalListResponse, original_data: dict
    ):
    ...
    def test_capacity_area_category_list_response_validation(
        self, response_examples_path
    ):
            ...
            if "items" in data and isinstance(data["items"], list):
                if True:
                    ...
                    try:
                        ...
                        self._validate_capacity_area_category_list_response_fields(
                            model_instance, data
                        )
                        print(f"✅ Validated {filename} as list response")
                    except ValidationError as e:
                        pytest.fail(
                            f"CapacityAreaCategoryListResponse validation failed for {filename}: {e}"
                        )
                    ...
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            print(
                                f"✅ Validated {filename} item {idx} with CapacityAreaCategory"
                            )
                        except ValidationError as e:
                            pytest.fail(
                                f"CapacityAreaCategory validation failed for {filename} item {idx}: {e}"
                            )
                else:
                    # For non-list models, validate individual items using the same model
                    for idx, item in enumerate(data["items"][:3]):
                        try:
                            ...
                            pytest.fail(
                                f"CapacityAreaCategoryListResponse validation failed for {filename} item {idx}: {e}"
                            )
            else:
                # Validate single response
                try:
    ...
    def _validate_capacity_area_category_list_response_fields(
        self, model: CapacityAreaCategoryListResponse, original_data: dict
    ):
