...
class TestAllResponseExamples:
    ...
    def test_validate_all_response_examples(self, response_examples_path):
        ...
        results = {"passed": [], "failed": [], "skipped": [], "not_implemented": []}
        ...
        for filename, model_class in self.RESPONSE_MODEL_MAPPING.items():
            ...
            try:
                ...
                if issubclass(model_class, BaseOFSResponse) or issubclass(
                    model_class, OFSResponseList
                ):
                ...
                results["failed"].append(
                    (filename, f"Unexpected error: {type(e).__name__}: {str(e)}")
                )
        ...
        assert len(results["failed"]) == 0, (
            f"{len(results['failed'])} models failed validation"
        )
    ...
    def test_model_coverage_statistics(self, response_examples_path):
        ...
        total_examples = len(
            [
                f
                for f in response_examples_path.glob("*.json")
                if f.name not in ["swagger.json", "endpoints.json"]
            ]
        )

        implemented = len(
            [m for m in self.RESPONSE_MODEL_MAPPING.values() if m is not None]
        )
        not_implemented = len(
            [m for m in self.RESPONSE_MODEL_MAPPING.values() if m is None]
        )

        coverage_pct = (
            (implemented / len(self.RESPONSE_MODEL_MAPPING)) * 100
            if self.RESPONSE_MODEL_MAPPING
            else 0
        )
        ...
        # Group by API
        by_api = {"Metadata": 0, "Core": 0, "Capacity": 0, "Statistics": 0, "Other": 0}

        for filename, model in self.RESPONSE_MODEL_MAPPING.items():
            if model is None:
                continue

            if "capacity" in filename.lower() or filename.startswith(
                ("14_", "16_", "17_", "19_", "20_", "21_", "23_")
            ):
                by_api["Capacity"] += 1
    ...
    def test_base_response_compliance(self, response_examples_path):
        ...
        for filename, model_class in self.RESPONSE_MODEL_MAPPING.items():
            ...
            # Check inheritance
            if not (
                issubclass(model_class, BaseOFSResponse)
                or issubclass(model_class, OFSResponseList)
                or model_class in [EnumerationValueList]
            ):  # Special cases
