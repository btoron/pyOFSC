...
class TestCollaborationEndpointsSchemaCompatibility(EndpointSchemaTestBase):
    ...
    def setup_method(self):
        ...
        self.collaboration_get_endpoints = self.get_endpoints_for_module(
            self.module_name
        )
...
class TestPartsCatalogEndpointsSchemaCompatibility(EndpointSchemaTestBase):
    ...
    def setup_method(self):
        ...
        self.partscatalog_get_endpoints = self.get_endpoints_for_module(
            self.module_name
        )
...
class TestOverallSchemaCompatibility(EndpointSchemaTestBase):
    ...
    def test_overall_coverage_report(self):
        ...
        overall_coverage = (
            (total_validated / total_endpoints * 100) if total_endpoints > 0 else 0
        )

        print("-" * 80)
        print(
            f"{'TOTAL':>15}: {total_validated:>3}/{total_endpoints:>3} "
