...
class TestMetadataModels:
    ...
    def test_properties_model_validation(self, response_examples_path):
        ...
        if paginated_file.exists():
            ...
            try:
                ...
                print(
                    f"✅ Validated paginated Property response with {len(response.items)} items"
                )
            except ValidationError as e:
        ...
        if paginated_file.exists():
            ...
            if "items" in data:
                try:
                    ...
                    assert len(response.items) >= 0
                    print(
                        f"✅ Validated paginated Workzone response with {len(response.items)} items"
                    )
                except ValidationError as e:
    ...
    def test_activity_types_model_validation(self, response_examples_path):
            ...
            if "items" in data and data["items"]:
                for item in data["items"][:3]:  # Test first 3 items
                    try:
                        ...
                        pytest.fail(
                            f"Activity type validation failed for {filename}: {e}"
                        )
        ...
        if paginated_file.exists():
            ...
            try:
                ...
                assert hasattr(response, "totalResults")
                print(
                    f"✅ Validated ActivityTypeListResponse with {len(response.items)} items"
                )
            except ValidationError as e:
    ...
    def test_work_skills_model_validation(self, response_examples_path):
        ...
        if "items" in data and data["items"]:
            for item in data["items"][:3]:  # Test first 3 items
                try:
                    ...
                    print(
                        f"✅ Validated work skill: {workskill.label} (sharing: {workskill.sharing})"
                    )
                except ValidationError as e:
        ...
        try:
            ...
            assert len(response.items) > 0
            print(
                f"✅ Validated paginated Workskill response with {len(response.items)} items"
            )
        except ValidationError as e:
    ...
    def test_metadata_examples_completeness(self, response_examples_path):
        ...
        for filename in required_examples:
            ...
            assert file_path.stat().st_size > 0, (
                f"Metadata example is empty: {filename}"
            )
    ...
    def test_timeslots_model_validation(self, response_examples_path):
        ...
        timeslots_files = ["67_get_time_slots.json", "67_timeslots.json"]
        ...
        for filename in timeslots_files:
            ...
            try:
                ...
                for i, timeslot in enumerate(
                    timeslots_response.items[:3]
                ):  # Test first 3
                    print(f"   TimeSlot {i + 1}: {timeslot.label} - {timeslot.name}")
                    print(
                        f"     Active: {timeslot.active}, All Day: {timeslot.isAllDay}"
                    )
                    ...
                    if timeslot.isAllDay:
                        ...
                        print(
                            f"     ✅ Timed slot: {timeslot.timeStart} - {timeslot.timeEnd}"
                        )
