...
class TestCapacityModels:
        ...
        if "items" in data and data["items"]:
            for item in data["items"][:3]:  # Test first 3 items
                try:
                    ...
                    assert (
                        capacity_category.label is not None
                    )  # CapacityCategoryResponse uses 'label', not 'name'
                    ...
                    if hasattr(capacity_category, "timeSlots"):
                        ...
                        assert capacity_category.timeSlots is None or hasattr(
                            capacity_category.timeSlots, "__iter__"
                        )
    ...
    def test_capacity_examples_completeness(self, response_examples_path):
        ...
        for filename in required_examples:
            ...
            assert file_path.stat().st_size > 0, (
                f"Capacity example is empty: {filename}"
            )
    ...
    def test_capacity_json_structure(self, response_examples_path):
        ...
        capacity_files.extend(
            list(response_examples_path.glob("14_*.json"))
        )  # Capacity areas
        capacity_files.extend(
            list(response_examples_path.glob("23_*.json"))
        )  # Capacity categories
