...
class TestAllModelsIntegration:
    ...
    def test_models_have_base_ofs_response_attributes(self):
        ...
        property_model = PropertyResponse(
            label="TEST", name="Test", entity="activity", type="string"
        )
    ...
    def test_comprehensive_response_validation(self, response_examples_path):
        ...
        assert passed_tests >= 5, (
            f"Expected at least 5 successful validations, got {passed_tests}"
        )
    ...
    def test_model_documentation_and_type_hints(self):
        ...
        for model in models_to_check:
            ...
            assert hasattr(model, "__annotations__"), (
                f"{model.__name__} missing type annotations"
            )
            assert len(model.__annotations__) > 0, (
                f"{model.__name__} has no field annotations"
            )

            # Check that model has ConfigDict
            assert hasattr(model, "model_config"), (
                f"{model.__name__} missing model_config"
            )
