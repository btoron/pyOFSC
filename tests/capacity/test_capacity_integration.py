...
class TestCapacityModuleIntegration:
    ...
    def test_available_capacity_with_categories(
    ...
    def test_capacity_different_intervals(self, ofsc_instance, test_dates, real_areas):
        ...
        intervals_to_test = [("all", "all"), ("15", "all"), ("all", "60")]
    ...
    def test_capacity_multiple_dates(self, ofsc_instance, test_dates, real_areas):
        ...
        # Test capacity
        capacity_response = ofsc_instance.capacity.getAvailableCapacity(
            dates=test_dates, areas=real_areas
        )
    ...
    def test_concurrent_capacity_requests(self, ofsc_instance):
        ...
        def make_capacity_request(area):
            ...
            return ofsc_instance.capacity.getAvailableCapacity(
                dates=[today_str], areas=[area]
            )

        def make_quota_request(area):
            """Make a quota request for a specific area"""
            return ofsc_instance.capacity.getQuota(dates=[today_str], areas=[area])
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
            capacity_futures = [
                executor.submit(make_capacity_request, area) for area in areas_to_test
            ]
            quota_futures = [
                executor.submit(make_quota_request, area) for area in areas_to_test
            ]
        ...
        print(
            f"Concurrent requests ({len(areas_to_test)} areas, both APIs): {concurrent_time:.2f}s"
        )
